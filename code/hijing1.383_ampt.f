...
C
C****************************************************************
        SUBROUTINE HIJING(EFRM,FRAME,BMIN0,BMAX0) !xj17
...
clin-7/2011 ioscar value is needed:
      ...
      COMMON /RUN/ NUM !XJ: TEST PARTICLES 
        SAVE   
c         real*4 nAch(50),niso(50)

	!xj17begin
           DD1=dble(EFRM)
           DD2=dble(0.939)
           beta=DSQRT(1.D0-4.D0*DD2**2/DD1**2)
		 gamma = 1./dsqrt(1.d0-beta**2)
c	write(*,*) gamma
c	stop
	!xj17end
...
C
20171	continue !xj trigger parton isospin
        YP(1,1)=0.0
        YP(2,1)=0.0
        YP(3,1)=0.0
...
clin-6/2009 write out initial transverse positions of initial nucleons:
        ...
        DO 201 JP=1,IHNT2(1)
clin-6/2009:
ctest on           write(94,203) YP(1,JP)+0.5*BB, YP(2,JP), JP, NFP(JP,5)
...
ctest on           write(94,203) YT(1,JT)-0.5*BB, YT(2,JT), -JT, NFT(JT,5)
       write(94,203) YT(1,JT)-0.5*BB, YT(2,JT), -JT, NFT(JT,5),yt(3,jt)
           ...
           ENDIF
 202    continue
ctest on 203    format(f10.3,1x,f10.3,2(1x,I5))
...
C       ****** identify the mother particle
                   ...
                   IF (NTP .EQ. 1) THEN
                      ...
                      GYAR(NATT) = YP(2, jjtp)
                      GZAR(NATT) = YP(3, jjtp)/gamma !xj17
                   ELSE
                      GXAR(NATT) = YT(1, jjtp)-0.5 * BB
                      GYAR(NATT) = YT(2, jjtp)
                      GZAR(NATT) = YT(3, jjtp)/gamma !xj17
                   END IF
cxj17                   GZAR(NATT) = 0.
                   FTAR(NATT) = 0.
                   ITYPAR(NATT) = K(I, 2)
...
cxj        CALL ZPCMN
	!replace ZPC with NJL
	CALL GETNJL(EFRM,IXJ) !XJ: TRANSFER PARTON DATA
c	write(*,*) ixj
	endif !xj
	ENDDO !XJ: TEST PARTICLES
...
c.....transfer data back from ZPC to HIJING
 ...
 1017      CONTINUE
  			NJSGS(I, IXJ)    = 0 !xj
 1018   CONTINUE
        DO 1019 I = 1, njlmul(ixj)
           IITYP=ITYPnjl(I,ixj)
           NSTRG = LSTRGnjl(I,ixj)
		 NJSGS(NSTRG, IXJ) = NJSGS(NSTRG, IXJ) + 1 !XJ
           NPART = LPARTnjl(I,ixj)
           K2SGS(NSTRG, NPART, IXJ) = ITYPnjl(I,ixj)
           PXSGS(NSTRG, NPART, IXJ) = PXnjl(I,ixj)
           PYSGS(NSTRG, NPART, IXJ) = PYnjl(I,ixj)
           PZSGS(NSTRG, NPART, IXJ) = PZnjl(I,ixj)
           PMSGS(NSTRG, NPART, IXJ) = XMASSnjl(I,ixj)
...
c     1          write(91,*) 'a',PX5(i),PY5(i),XMASS5(i),PZ5(i),E5(i)
           PESGS(NSTRG, NPART, IXJ) = Enjl(I,ixj)
c           if(abs(PZ5(i)/E5(i)).gt.0.9999999d0) 
c     1          write(91,*) 'b: new E5(I)=',E5(i)
clin-7/20/01-end
           GXSGS(NSTRG, NPART, IXJ) = GXnjl(I,ixj)
           GYSGS(NSTRG, NPART, IXJ) = GYnjl(I,ixj)
           GZSGS(NSTRG, NPART, IXJ) = GZnjl(I,ixj)
           FTSGS(NSTRG, NPART, IXJ) = FTnjl(I,ixj)
 1019   CONTINUE
c        CALL HJANA2
	ENDDO
clin-4/19/01-end
...
clin-4/30/01 convert partons to hadrons after ZPC:
	...
	MULTI1(IXJ) = MULTI1(IXJ) + 1
              GXARXJ(MULTI1(IXJ),IXJ)=GXNxj(I,ixj)
              GYARXJ(MULTI1(IXJ),IXJ)=GYNxj(I,ixj)
              GZARXJ(MULTI1(IXJ),IXJ)=GZNxj(I,ixj)
              FTARXJ(MULTI1(IXJ),IXJ)=FTNxj(I,ixj)
              ITYPARXJ(MULTI1(IXJ),IXJ)=ITYPNxj(I,ixj)
              PXARXJ(MULTI1(IXJ),IXJ)=PXNxj(I,ixj)
              PYARXJ(MULTI1(IXJ),IXJ)=PYNxj(I,ixj)
              PZARXJ(MULTI1(IXJ),IXJ)=PZNxj(I,ixj)
              PEARXJ(MULTI1(IXJ),IXJ)=EENxj(I,ixj)
              XMARXJ(MULTI1(IXJ),IXJ)=XMNxj(I,ixj)
 1006      continue
	ENDDO
        endif
...
clin-4/19/01 soft3:
 565    continue
	RETURN !ENERGY CONSERVATION MAY BE A PROBLEM, XJ
